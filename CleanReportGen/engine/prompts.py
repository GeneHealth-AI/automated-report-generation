...
class BlockTemplate:
    ...
    INTRODUCTION: Dict[str, Any] = {
        ...
        - Report Scope: Say relevant genes for the condition without being overly verbose.
    ...
    CLINICAL_IMPLICATIONS = {
        ...
        - Known Drugs:
          ...
          * Provide informational context about commonly prescribed drugs for {category} risk (e.g. for pancreatic: enzyme replacement, pain management, or specific targeted therapies if applicable).
    ...
    LIFESTYLE_RECOMMENDATIONS = {
        ...
        "llm_prompt": """
        Generate "Pancreatic Lifestyle Recommendations" for risk reduction.
        ...
        - Diet modifications
        - Exercise
        - Smoking/Alcohol (critical for pancreatic)
    ...
    STUDY_SELECTION = {
        "llm_prompt": """
        Given the following list of PubMed studies (PMID, Title, Abstract), select only the studies that are highly relevant to {category} and the specific focus on {mutation_context}.
        
        Studies:
        {studies_text}
        
        Return a JSON object with a list of relevant PMIDs:
        {{
            "relevant_pmids": ["PMID1", "PMID2", ...]
        }}
        """,
        "required_data": ["category", "mutation_context", "studies_text"]
    }

    STUDY_SUMMARIZATION = {
        "llm_prompt": """
        Summarize the following study in the context of {category} and the mutation/protein {mutation_context}.
        
        Title: {title}
        Abstract: {abstract}
        
        Provide a concise summary of the results and explain exactly why this study is relevant to the patient's specific profile.
        
        Return JSON:
        {{
            "pmid": "{pmid}",
            "summary": "Concise summary of results",
            "relevance": "Explanation of relevance to {mutation_context}"
        }}
        """,
        "required_data": ["category", "mutation_context", "title", "abstract", "pmid"]
    }
    
    @classmethod
    def get_template(cls, block_type: BlockType):
        """Helper to get template metadata by Enum."""
